...
def run_scripts_parallel(script_paths: list[Path], extra_args: list[str]) -> int:
    procs = []
    ...
    first_non_zero = next((code for code in exit_codes if code != 0), 0)
...
def main() -> int:
    ...
    # "all" with long-running schedulers should run both scrapers at the same time.
    if "--once" not in passthrough:
        return run_scripts_parallel([INTERNSHIP_SCRAPER, MENTORSHIP_SCRAPER], passthrough)
